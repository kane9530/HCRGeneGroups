...
for (i in 1:length(my_csv_files_noto)){
    my_csv_processed_noto <- processcsvnoto(my_df = my_csv_files_noto[[i]][[1]], 
                                        gene_names = gene_names[2],
                                        my_sample_noto = my_sample_noto[i])
    my_csv_files_proc_noto[[i]] <- my_csv_processed_noto

    #my_csv_files_proc_merged_noto[[i]] <- my_csv_files_proc_noto[[i]][[1]]
...
####EXPORT THE DAMN SOX VALUES AS WELLLLLL AHHHHHHHHH


df_combined_noto <- my_csv_files_proc_noto %>%
  reduce(rbind) %>%
  group_by(sample_ID) %>%
  arrange(sample_ID, cell_ID)%>%
  mutate_at(.funs = list(normalise = ~((.-min(.))/max(.-min(.)))), .vars = 3:4) %>%
